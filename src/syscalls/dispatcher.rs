...
//
// Production wires the closures to the live page-table walk + HHDM deref
// inside `read_user_buffer` / `write_user_buffer` — the only public entry
// points used by syscall handlers. Callsites are unchanged from Cut 1.
...
///
/// Production wrapper: walks the process page table (CR3) to translate
/// each page the buffer spans, then reads via HHDM. Returns the number
/// of bytes copied. cr3=0 yields a translator that always returns None,
/// so the first byte fails with `InvalidArg` (preserves Cut 1 semantics).
///
/// # Safety contract
...
mod tests {
    ...
    // ========================================================================
    //
    // Drives the page-boundary chunking, partial-failure handling, and
    // translate-call accounting inside the generic workers. A `FakeSpace`
    // backs both the translator and the read/write_phys closures, using
    // identity translation (vaddr == phys) so test reasoning matches the
    // assertions: "writes at vaddr 0x1500" lands at "page 0x1000 offset 0x500".
