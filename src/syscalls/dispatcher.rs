...
impl SyscallDispatcher {
    ...
    fn handle_exit(args: SyscallArgs, ctx: &SyscallContext) -> SyscallResult {
        ...
        // ADR-019 § Decision 1: one reap path, shared with the fault
        // handlers. Everything from the Terminated transition through the
        // reaper enqueue lives in `crate::reap::reap_process`.
        crate::reap::reap_process(
            ctx.process_id,
            ctx.task_id,
            ctx.caller_principal,
            crate::reap::ExitReason::Exited(exit_code as i32),
        );
    ...
    pub(crate) fn teardown_channel_mappings(record: &crate::ipc::channel::ChannelRecord) {
    ...
    /// needs.
    pub(crate) fn do_cluster_revoke(
