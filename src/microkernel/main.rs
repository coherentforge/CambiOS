...
unsafe extern "C" fn kmain() -> ! {
    ...
    // Spin delay to let serial output fully flush before proceeding
    for _ in 0..5_000_000u64 { core::hint::spin_loop(); }

    println!("[DBG] marker 1: before heap init");
    for _ in 0..5_000_000u64 { core::hint::spin_loop(); }

    // Initialize kernel heap allocator from Limine memory map
    init_kernel_heap();

    println!("[DBG] marker 2: after heap init");
    for _ in 0..5_000_000u64 { core::hint::spin_loop(); }
    println!("Heap initialized successfully.");

    // Initialize core hardware (IDT, etc.)
    println!("[DBG] initializing IDT...");
    for _ in 0..5_000_000u64 { core::hint::spin_loop(); }
    unsafe {
        arcos_core::interrupts::init();
    }
    println!("[DBG] IDT done");
    for _ in 0..5_000_000u64 { core::hint::spin_loop(); }

    // Quick heap allocation test
    {
        println!("[DBG] heap test: allocating Box<u64>...");
        let test = Box::new(42u64);
        println!("[DBG] heap test: *test = {}", *test);
        drop(test);
        println!("[DBG] heap test: deallocated OK");
        for _ in 0..5_000_000u64 { core::hint::spin_loop(); }

        println!("[DBG] heap test: allocating Box<[u8; 8192]>...");
        let big = Box::new([0xAAu8; 8192]);
        println!("[DBG] heap test: big[0]={:#x}, big[8191]={:#x}", big[0], big[8191]);
        drop(big);
        println!("[DBG] heap test: big deallocated OK");
        for _ in 0..5_000_000u64 { core::hint::spin_loop(); }
    }

    // Initialize microkernel subsystems
    println!("[DBG] scheduler_init...");
    scheduler_init();
    println!("[DBG] ipc_init...");
    ipc_init();
    println!("[DBG] capability_manager_init...");
    capability_manager_init();
    println!("[DBG] process_table_init...");
    process_table_init();
    println!("[DBG] register_example_interrupts...");
...
fn init_kernel_heap() {
    ...
    println!("[DBG] heap: getting HHDM");
    let hhdm_offset = HHDM_REQUEST
        .get_response()
        .expect("HHDM response required for kernel heap")
        .offset();

    println!("[DBG] heap: getting memmap");
    ...
    println!("[DBG] heap: calling init at virt {:#x}, size {:#x}", virt_base, heap_size);
    unsafe {
        arcos_core::KERNEL_HEAP.init(virt_base, heap_size);
    }
    println!("[DBG] heap: init returned");
...
fn scheduler_init() {
    ...
    println!("[DBG] sched: storing in global");
    // Store in spinlock-protected global (already heap-allocated)
    *SCHEDULER.lock() = Some(scheduler);
    println!("[DBG] sched: stored, creating timer");
...
fn ipc_init() {
    *IPC_MANAGER.lock() = Some(IpcManager::new_boxed());
    ...
    match num {
        ...
        SyscallNumber::Allocate => {
            ...
            if let Some(table) = pt.as_mut() {
                let addr = table.allocate_for(process_id, size);
                if addr == 0 {
        ...
        SyscallNumber::Free => {
            ...
            let mut pt = PROCESS_TABLE.lock();
            if let Some(table) = pt.as_mut() {
                if table.free_for(process_id, ptr) {
        ...
        SyscallNumber::RegisterEndpoint => {
            ...
            let cap_mgr = cap_guard.as_mut().unwrap();
