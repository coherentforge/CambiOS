...
///
/// File format: CKEY v1 (see [ADR-025](../../docs/adr/025-principal-as-aid.md)).
/// Header is parsed at compile time via `parse_bootstrap_pubkey_v1`; mismatched
/// magic / version / algo fails the build rather than producing a confused
/// trust root.
fn bootstrap_identity_init() {
    use cambios_core::ipc::Principal;

    // Bootstrap public key compiled in. Two sources depending on build mode:
    //
    //  - default (production): bootstrap_pubkey.bin — Jason's real YubiKey
    //    pubkey, committed to the repo. Generated once by
    //    `sign-elf --yubikey --export-pubkey bootstrap_pubkey.bin`.
    //
    //  - --features dev-piv (dev / CI / SwPivBackend integration):
    //    dev_bootstrap_pubkey.bin — per-developer pubkey derived from a
    //    persistent seed by `make gen-dev-piv-keys`. Pairs with
    //    `--features dev-piv` on `cambios-key-store-service` so the kernel's
    //    verification path agrees with `SwPivBackend`'s signing key. ADR-032
    //    Migration Path step 2; see notes/yubikey-stream-a-handoff.md.
    //
    // Either path produces a 40-byte CKEY v1 file; the parser below is the
    // same.
    #[cfg(not(feature = "dev-piv"))]
    const BOOTSTRAP_PUBKEY_FILE: &[u8] =
        include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/bootstrap_pubkey.bin"));
    #[cfg(feature = "dev-piv")]
    const BOOTSTRAP_PUBKEY_FILE: &[u8] =
        include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/dev_bootstrap_pubkey.bin"));
    const BOOTSTRAP_PUBKEY: [u8; 32] = parse_bootstrap_pubkey_v1(BOOTSTRAP_PUBKEY_FILE);
    const BOOTSTRAP_AID_PREFIX: [u8; 2] =
        parse_bootstrap_aid_prefix_v1(BOOTSTRAP_PUBKEY_FILE);
    ...
    println!(
        "  Principal: {} (AID prefix: {:02x}{:02x})",
        bootstrap, BOOTSTRAP_AID_PREFIX[0], BOOTSTRAP_AID_PREFIX[1]
    );
}

/// Parse the v1 CKEY-format bootstrap pubkey file at compile time, returning
/// the 32-byte Ed25519 public key. Fails the build via const-eval `panic!` if
/// the magic, version, or algo bytes do not match expectations.
///
/// File layout (40 bytes, see [ADR-025](../../docs/adr/025-principal-as-aid.md)):
/// `[magic:4 = "CKEY"][version:1 = 0x01][algo:1 = 0x00 (Ed25519)]
///  [aid_prefix:2 = blake3(key)[..2]][key:32]`
const fn parse_bootstrap_pubkey_v1(raw: &[u8]) -> [u8; 32] {
    // ARCHITECTURAL: 40-byte file = 8-byte CKEY v1 header + 32-byte Ed25519 key.
    if raw.len() != 40 {
        panic!("bootstrap_pubkey.bin: expected 40 bytes (CKEY v1 Ed25519). \
                Run `sign-elf --rewrap-pubkey bootstrap_pubkey.bin` to migrate \
                from the legacy 32-byte raw format.");
    }
    if !(raw[0] == b'C' && raw[1] == b'K' && raw[2] == b'E' && raw[3] == b'Y') {
        panic!("bootstrap_pubkey.bin: magic bytes are not 'CKEY' — file is not \
                a v1 CKEY pubkey. See docs/adr/025-principal-as-aid.md.");
    }
    if raw[4] != 1 {
        panic!("bootstrap_pubkey.bin: version byte != 1 — kernel only knows \
                CKEY v1.");
    }
    if raw[5] != 0 {
...
/// Extract the 2-byte AID prefix `blake3(key)[..2]` recorded in the CKEY v1
/// header. Informational only — used for boot-log fingerprinting and future
/// KERI key-id lookup. The kernel does not verify the prefix matches the key
/// at runtime; `sign-elf` is the source of truth for prefix correctness.
const fn parse_bootstrap_aid_prefix_v1(raw: &[u8]) -> [u8; 2] {
    if raw.len() != 40 {
        panic!("bootstrap_pubkey.bin: expected 40 bytes (CKEY v1 Ed25519).");
    }
    [raw[6], raw[7]]
