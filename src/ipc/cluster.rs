...
impl ClusterManager {
    ...
    /// box. `ClusterManager` is tens of KiB; on the riscv64 boot stack that
    ...
    pub fn new_boxed() -> Option<Box<Self>> {
        ...
        let slots = unsafe { core::ptr::addr_of_mut!((*ptr).clusters) } as *mut Option<ClusterRecord>;
        for i in 0..MAX_CLUSTERS {
            // SAFETY: i < MAX_CLUSTERS, so the projected element is in bounds.
