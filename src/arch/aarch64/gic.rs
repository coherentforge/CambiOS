...
pub unsafe fn init() {
    ...
    // SAFETY: Writing ICC_SRE_EL1 from EL1 during boot is safe.
    core::arch::asm!(
    ...
    // ICC_PMR_EL1 = 0xFF
    // SAFETY: Writing ICC_PMR_EL1 from EL1 is safe.
    core::arch::asm!(
        "mov {tmp}, #0xFF",
    ...
    // SAFETY: Enabling Group 1 interrupts from EL1 during boot is safe.
    core::arch::asm!(
        "mov {tmp}, #1",
...
unsafe fn gicr_read(cpu_id: u32, offset: usize) -> u32 {
    ...
    core::ptr::read_volatile((cpu_base + offset) as *const u32)
}
...
unsafe fn gicr_write(cpu_id: u32, offset: usize, value: u32) {
    ...
    let cpu_base = base as usize + (cpu_id as usize) * 0x20000;
    // SAFETY: base + cpu_id offset is the per-CPU GICR frame.
    core::ptr::write_volatile((cpu_base + offset) as *mut u32, value);
}
...
pub unsafe fn init_distributor(gicd_base: u64) {
    ...
    // SAFETY: GICD_BASE just set, GICD_CTLR is offset 0.
...
pub unsafe fn enable_spi(intid: u32) {
    ...
    gicd_write(GICD_ISENABLER + reg_idx * 4, bit);
}
...
pub unsafe fn disable_spi(intid: u32) {
    ...
    let reg_idx = (intid / 32) as usize;
    let bit = 1u32 << (intid % 32);
    gicd_write(GICD_ICENABLER + reg_idx * 4, bit);
}
