...
last_synced_to_code: 2026-05-07 (ADR-027 cluster syscall handlers)
...
| Service clusters | Kernel ABI live: `SYS_CLUSTER_CREATE` / `_JOIN` / `_REVOKE` / `_INFO` (44–47), `CapabilityKind::CreateCluster` / `ClusterRevoke`, `cluster_policy::caps_for_role` stub (no-op) + `on_member_depart` (RenderingLimb → revoke), exit-path auto-revoke. Bootstrap CreateCluster grant + rendering-limb migration pending. | x/a/r | `src/ipc/cluster.rs`, `src/ipc/cluster_policy.rs`, `src/syscalls/dispatcher.rs` (cluster handlers) | [ADR-027](docs/adr/027-service-clusters.md) |
