...
| Audit consumer capability + `audit-tail` | Done (commit chain on `audit-consumer-cap`; bootstrap-only check replaced) | x/a/r | `user/audit-tail/`, `src/ipc/capability.rs` (`AuditConsumer`), `src/syscalls/dispatcher.rs` (`handle_get_process_principal`), `src/process.rs` (`RecentExitsRing`) | [ADR-023](docs/adr/023-audit-consumer-capability.md) |
