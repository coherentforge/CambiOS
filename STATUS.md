...
last_synced_to_code: 2026-05-18 (YubiKey Stream A substage A-ii — `SwPivBackend` + dev-piv kernel cfg flag + `gen-dev-piv-keys` host tool)
...
| Multi-Principal vault service | Done (Phase 1C; userspace plurality, kernel-side Principal stays singular/atomic; AI sandbox is per-Principal) | x/a/r | `user/key-store-service/` (vault extends ep 17) | [identity.md](docs/identity.md) Phase 1C, [ADR-026](docs/adr/026-identity-transcription-at-the-kernel-ring.md) |
...
| Key-store service | Done (endpoint 17, degraded mode in default builds; `SwPivBackend` available behind `--features dev-piv`, IPC dispatch wires at A-iii) | x/a/r | `user/key-store-service/` | [ADR-032](docs/adr/032-full-disk-encryption-below-substrate.md) |
...
| USB host stack (xHCI) | In progress (Stream B B-i) — capability-register bring-up only. Discovers xHCI controller via PCI class=0x0C/sub=0x03/prog_if=0x30, maps MMIO BAR, parses cap block, idles on ep 31. No port enum, no transfers, no IRQ. | x/a/r (buildable; runtime detection requires `-device qemu-xhci`) | `user/usb-host/`, `src/pci/mod.rs` (`prog_if`) | — |
