...
last_synced_to_code: 2026-04-16 (Phase 4b: boot cleanup + arcobj end-to-end — hello.elf removed from boot chain, [Tick] heartbeat removed, virtio-blk now in AArch64 build, SYS_TRY_RECV_MSG=37 added for dual-endpoint polling, REPLY_ENDPOINT registry fixes IPC reply routing, lazy-disk init moved outside OBJECT_STORE lock; also: RISC-V Phase R-1 boots in QEMU virt — serial banner via OpenSBI + NS16550)
...
- **x86_64**: clean release build, boots in QEMU with 2 CPUs, 5 boot modules running (policy-service, key-store, fs-service, virtio-blk, shell). `arcobj list/put/get` works end-to-end; objects persist across reboots on virtio-blk.
- **AArch64**: clean release build, boots in QEMU `virt`, `make img-aarch64` now builds + includes virtio-blk (same 5-module limine.conf as x86_64). Driver's port-I/O init path is x86_64-specific — on AArch64 the driver enters `no_device_loop()` and ObjectStore stays RAM-backed until AArch64 device IRQ routing lands.
...
| **RISC-V (riscv64gc) port** | In progress (Phase R-1 done 2026-04-16) | Third architecture backend, parity-targeted with x86_64/AArch64. Phase R-1 milestone: `make run-riscv64` boots in QEMU virt under OpenSBI v1.5.1, prints `=== CambiOS Microkernel [v0.2.0] (RISC-V Phase R-1) ===` via NS16550 MMIO at 0x10000000, then halts on WFI. Implements: full `src/arch/riscv64/` skeleton (mod, percpu via `tp`, syscall stub, tlb/sfence.vma, plic, timer, entry/`_start` asm), NS16550 UART driver in `src/io/`, riscv64 arms in lib.rs (`local_*`, halt, wfi), policy/dispatcher/platform/loader/audit cfg arms, `boot::riscv::populate()` adapter (DTB walk deferred to R-2), gated AP-startup in main.rs (Phase R-5 SBI-based wake), `kmain_riscv64(hart_id, dtb_phys)` entry. **Linker layout deviates from plan**: kernel runs at `0x80200000` physical = virtual with paging disabled in R-1; the higher-half + Sv48 boot trampoline lands in R-2 (per ADR-013 Decision 3 fallback paragraph + R-1 risk #1). Bootloader = OpenSBI + custom S-mode `_start` (no Limine on RISC-V). Hardware target = TBD CambiOS-designed; QEMU virt for dev. | `src/arch/riscv64/`, `src/boot/riscv.rs`, `src/microkernel/main.rs::kmain_riscv64`, `linker-riscv64.ld`, `Makefile` | [ADR-013](docs/adr/013-riscv64-architecture-support.md) |
...
| **R-1** | First serial output. OpenSBI hand-off, S-mode `_start` boot stub, NS16550 UART, minimal `kmain_riscv64(hart_id, dtb_phys)` reaches println. | **Done (2026-04-16)** — kernel boots in QEMU virt, prints banner, halts. Sv48 boot trampoline + DTB parser pulled forward into R-2 per ADR-013 Decision 3 fallback (R-1 runs at `0x80200000` physical=virtual with paging off). |
| **R-2** | Memory management. **Sv48 boot page table trampoline pulled in from R-1.** Higher-half kernel mapping at `0xffffffff80000000`. Hand-rolled DTB parser for `/memory`, `/chosen`, `/cpus`. RISC-V PTE bit constants in shared paging module, `satp`-based `active_page_table()`, frame-allocator init with RISC-V reservations (OpenSBI, DTB, kernel image). | Planned |
