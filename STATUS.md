...
- **Formal verification**: Kani proofs live on BuddyAllocator + ELF parser + CapabilityManager (20 harnesses across 3 proof crates). Frame allocator queued next.
...
| PCI bus discovery | Done (cross-arch; virtio-mmio on riscv64) | x/a/r | `src/pci/` | — |
...
| Compositor (incl. Input-1 event routing) | Done (Scanout-2/3 + Input-1) | x | `user/compositor/` | [ADR-014](docs/adr/014-compositor-scanout-driver-protocol.md) |
| scanout-virtio-gpu driver (default) | Done (Scanout-4.b) | x | `user/scanout-virtio-gpu/` | [ADR-014](docs/adr/014-compositor-scanout-driver-protocol.md) |
...
| tree v0 (first-party app, default GUI boot module) | Done | x | `user/tree/` | [ADR-011](docs/adr/011-graphics-architecture-and-scaling.md), [ADR-012](docs/adr/012-input-architecture-and-device-classes.md), [ADR-014](docs/adr/014-compositor-scanout-driver-protocol.md) |
| hello-window | Done, buildable; retained as protocol regression (replaced by `tree` as default GUI boot module) | x | `user/hello-window/` | [ADR-011](docs/adr/011-graphics-architecture-and-scaling.md), [ADR-014](docs/adr/014-compositor-scanout-driver-protocol.md) |
...
| virtio-input driver | Done (Input-1) | x | `user/virtio-input/` | [ADR-012](docs/adr/012-input-architecture-and-device-classes.md) |
...
| Formal verification (Kani) | Started 2026-04-16. Live: `BuddyAllocator::free` reserved-prefix (150 checks, ~18s); ELF header parser in `src/loader/elf.rs` (7 harnesses; proof authoring found + fixed 6 integer-overflow sites); capability manager in `src/ipc/capability.rs` (12 harnesses — Tier A `ProcessCapabilities` state-machine invariants + Tier B cross-process properties on a 3-slot `Box::leak`'d manager: stale-generation rejection, no-delegate-without-right, no-escalation, `revoke_all` clears endpoint + system caps, non-bootstrap revoke denied). 20 passing harnesses across 3 proof crates. Next by leverage: frame allocator bitmap ops. Compositor protocol parser deferred until scanout settles past Scanout-4.c. | — | `verification/buddy-proofs/`, `verification/elf-proofs/`, `verification/capability-proofs/` | [ADR-000 § Divergence](docs/adr/000-zta-and-cap.md) |
