...
- **2026-05-03** - Convention 10 "Exceptions in practice" subsection added after the kernel-tree alphabetical sweep (commits f24233e through a15a743) hit five recurring patterns the original prose did not cover: Convention 9 triggers preserve phase numbers, runtime log strings are out of scope, ADR prose is out of scope, within-function step labels rename to "Step N" rather than getting dropped, and `Phase X (substantive-subsystem)` parentheticals lose the phase tag while keeping the substantive name. Each exception cites its kernel-sweep precedent so future maintainers can see the rule applied. The earlier 2026-05-03 entry (immediately below) records the rule's introduction.
- **2026-05-03** - Convention 10: code comments anchor to durable things (ADR IDs, substantive reasons), never to roadmap phase numbers. Found ~45 phase references in `src/microkernel/main.rs` alone (`Phase 3.2a..3.4`, `R-1..R-6`, `ADR-021 B.1..B.3`, `GUI-0+`, `Scanout-2`); none survive without STATUS.md context that has moved on. Same anti-rot shape as the 2026-04-16 "no forward numeric ADR references" rule and Convention 9: durable artifacts must not anchor to mutable indices. Sweep done file-by-file across the kernel tree starting with main.rs. Calibration on the surrounding question "should we always also describe what the code does?" landed at: no, that pulls toward stale-comment territory; what delivers easier-to-contend-with-later is more *ADR refs* and more *WHY-comments for non-obvious constraints*, not more WHAT-comments. Existing CLAUDE.md "default to no comments; explain WHY not WHAT" preserved unchanged. Lint deferred per usual cadence (`make check-phase-refs` flagging `Phase \d` patterns lands on first recurrence).
- **2026-05-02** — H1 `Staged files:` parser rejects column-zero `- path` blocks; the parser requires path lines indented past the header in both bullet and bare forms. The 2026-05-01 loosening accepted `- path` *or* bare `path`, but the indent-past-header requirement was unchanged and undocumented in that changelog entry. Claude (this session) read "loosened to accept bullet form" as "indent optional" and produced a column-zero block; H1 fired with the parse-zero "Staged but not listed in commit body" complaint, which is correct but misleading — the body *looked* fine; the parser had silently rejected every line for being at the header's indent level. Cost: one wasted commit cycle. **Diagnostic-only mechanism candidate:** add one line to the H1 error block — "(path lines must be indented past the header — the bullet `-` is part of the path token, not the header column)". Parser semantics unchanged. **Revisit when:** next H1 bite of this shape, or next time `tools/check-claude-staged-files.py` is touched.
- **2026-05-02** — Pre-push edit-window gate (`tools/check-edit-window.py`, 2026-04-25 entry) fired a false positive on a legitimately-authored commit. Root cause: `CLAUDE_PREFLIGHT_SESSION` was not exported in this session, so `tools/log-claude-edit.py:90-92` silently no-op'd every Edit/Write — including the `Write` of `docs/adr/026-identity-transcription-at-the-kernel-ring.md`. The pre-push gate read prior sessions' jsonl logs from `.git/claude-edit-log/` and saw ADR-026 absent; gate enforced. The 2026-04-25 changelog entry says "warn-but-pass without an edit log," but the implementation enforces as long as *some* log file exists — even if no log exists for *this* session. The asymmetry: gate keys on directory-wide presence of any log, not on the current session's log presence. **Mechanism candidates:** (a) `tools/check-edit-window.py` checks `.git/claude-edit-log/${CLAUDE_PREFLIGHT_SESSION}.jsonl` exists before enforcing; missing → warn-but-pass per the original promise. (b) `tools/log-claude-edit.py` emits a one-line "session logging disabled (CLAUDE_PREFLIGHT_SESSION unset)" notice on first invocation so the silent no-op surfaces before push, not at push time. **Revisit when:** second false positive, or next pre-push gate touch. Bypass for this push was `git push --no-verify`; the file was independently confirmed as a session artifact via conversation transcript.
- **2026-05-01** — Loosened the H1 `Staged files:` parser (`tools/check-claude-staged-files.py`) to accept Markdown bullet form (`- path` / `* path`) in addition to bare-indented paths. Why: the bullet-trap kept biting Claude — Markdown convention reaches for `- ` and the parser's pre-2026-05-01 behavior treated the dash+space as internal whitespace, ending the block early and rejecting the commit. The intent of H1 is *file-list correspondence* (claimed paths == staged paths) — the bare-vs-bulleted choice was incidental to the implementation, not load-bearing. Loosening preserves the prose-continuation rejection: a line like `- This change adds something` still ends the block because the post-bullet remainder retains internal whitespace. Verified via five synthetic fixtures (legacy bare form, new bullet form, prose-continuation rejection, missing-block rejection, non-Claude pass-through). Memory note `feedback_claude_staged_files_block.md` updated to show both forms as canonical; old "no bullets" pitfall demoted to a historical note. Lesson on changelog convention: when a rule's failure mode keeps recurring not because the rule is wrong but because the *enforcement shape* is unergonomic, loosen the enforcement, don't pile on yet another reminder.
- **2026-04-26** — Convention 1 extended to `pub unsafe fn` signatures via `clippy::missing_safety_doc = "deny"` in `Cargo.toml [lints.clippy]`. Tier 1 E2 path (a) of [`~/.claude/plans/scope-tier-2-also-prancy-manatee.md`](../../.claude/plans/scope-tier-2-also-prancy-manatee.md). The plan agent's pre-scan claimed 135 `pub unsafe fn` sites lacked `# Safety:` rustdoc; the actual count after enabling the lint on a clean build was **zero** — pre-existing discipline already covered the rule's intent. Verified the lint was engaged by temporarily stripping `# Safety:` from `src/io/mod.rs::init` (clippy flagged it) and reverting (clippy clean again). The rule extension is therefore mechanical only: lock in what was already practice. Companion to E1+E3's `make check-unsafe-coverage`; together they enforce Convention 1 at both the block level (lint) and the signature level (clippy). Future regressions on either side fail the relevant gate.
- **2026-04-25** — `make check-unsafe-coverage` lint + empty baseline, wired into `.githooks/pre-commit`. Enforces Development Convention 1 ("every unsafe block MUST have a `// SAFETY:` comment"). The literal rule has been in CLAUDE.md since the start; without enforcement, drift was inevitable. Comment-block-aware Python walker handles the three idioms that an early naive scanner over-flagged: (i) **cluster** — a single SAFETY block above a sequence of adjacent `let X = unsafe { ... };` reads covers the whole cluster; (ii) **assignment continuation** — `let foo: Bar =\n    unsafe { ... }` keeps the SAFETY contiguous across the `=`-terminated line; (iii) **outer attributes** — `#[cfg(target_arch = "x86_64")]` between SAFETY and unsafe doesn't break contiguity. With those three rules, the initial 23-flag count dropped to 4 real violations (E1), all fixed inline. Don't-grow-the-baseline pattern same as `check-boot-panics` / `check-assumptions` / `check-deferrals`. Tier 1 E1 + E3 of [`~/.claude/plans/scope-tier-2-also-prancy-manatee.md`](../../.claude/plans/scope-tier-2-also-prancy-manatee.md). E2 (extending the rule to `pub unsafe fn` rustdoc `# Safety:` sections) deliberately deferred — landing it pre-HN would touch ~135 sites; the lint mechanism is now in place to ratchet that in later.
- **2026-04-25** — Added `make check-lockfile` lint + `tools/check-lockfile-additions.py` and wired into `.githooks/pre-commit`. T-1 in [docs/threat-model.md](docs/threat-model.md): a poisoned transitive Cargo dep runs arbitrary code in `build.rs` / proc macros on the developer host before any signature is checked. Pre-CI / pre-HN we don't block lockfile changes — legitimate `cargo update` and intentional new deps need to flow — but every commit that touches `Cargo.lock` now prints the (crate, version) delta to stderr so the human committer (or PR reviewer post-HN) sees what entered the tree. Same posture as the existing pre-commit lints: noisy enough to read, not bossy enough to block. Tightening to a hard block becomes worth it the moment external PRs start arriving via a forge that runs `cargo build` on fork CI; the threat-model entry's Revisit-when ("first external contributor") is also the trigger to layer in `cargo-vet` for actual per-(crate, version) audit state.
