...
13. **AArch64 port (implementation)** — Fill in `todo!()` stubs with real assembly: GICv3 MMIO register access, ARM Generic Timer (CNTV), PL011 UART, AArch64 exception vector table, 4-level page tables (different entry format), context save/restore. Requires `aarch64-unknown-none` target + separate linker script + QEMU `virt` machine for testing.
