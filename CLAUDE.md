...
- **2026-05-02** - H1 `Staged files:` parser (`tools/check-claude-staged-files.py`) rejects column-zero `- path` blocks; the parser requires path lines indented past the header in both bullet and bare forms. Failure mode: H1 fires the "Staged but not listed in commit body" complaint, which is correct but misleading - the body *looks* fine; the parser silently rejected every line for being at the header's indent level. Cost when this misfires: one wasted commit cycle. **Diagnostic-only mechanism candidate:** add one line to the H1 error block - "(path lines must be indented past the header - the bullet `-` is part of the path token, not the header column)". Parser semantics unchanged. **Revisit when:** next H1 bite of this shape, or next time `tools/check-claude-staged-files.py` is touched.
