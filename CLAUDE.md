...
# Run tests (214 tests, all passing)
...
- **Voluntary context switch** via `yield_save_and_switch` (x86_64 assembly in `arch/x86_64/mod.rs`): builds synthetic iretq frame + all GPRs (identical SavedContext to timer ISR), calls `yield_inner` → `on_voluntary_yield` → `scheduler.voluntary_yield()`. No APIC EOI (not a hardware interrupt). Handles TSS/CR3 updates on switch. Used by handle_exit, handle_yield, handle_recv_msg (restart loop), handle_wait_irq.
- **Blocking pattern**: `cli` → `block_task(Blocked)` → `yield_save_and_switch()` → re-check on wake. The `cli` before `block_task` prevents the timer ISR from seeing Blocked state before yield saves correct saved_rsp.
...
- ~~**Terminated tasks re-fault after SYS_EXIT.**~~ FIXED. `handle_exit` marks the task Terminated and calls `yield_save_and_switch()`, which saves context and schedules the next task. The terminated task is never re-scheduled. On AArch64, the SVC handler still uses `halt_until_preempted` as a fallback (AArch64 yield_save_and_switch is a wfi stub pending full port).
...
- ~~**No voluntary context switch.**~~ FIXED. `yield_save_and_switch` (x86_64 assembly trampoline) builds a synthetic iretq frame (identical SavedContext layout to timer ISR) on the kernel stack, calls `yield_inner` → `on_voluntary_yield` → `scheduler.voluntary_yield()`, then restores from the next task's SavedContext via iretq. SYSCALL entry now switches to the kernel stack via `PerCpu.kernel_rsp0` (gs:[24]) and enables interrupts (sti) so handlers are preemptible. Blocking handlers (recv_msg, wait_irq) use a restart loop: `cli` → `block_task` → `yield_save_and_switch` → re-check on wake. The `cli` before `block_task` prevents the timer ISR from seeing Blocked state before yield saves the correct saved_rsp. AArch64 `yield_save_and_switch` is a wfi stub pending full SVC kernel-stack entry port.
...
24. ~~**Crypto Integration (Phase 1B)**~~ — DONE. Added `ed25519-compact` 2.2 + `blake3` 1.8 crates (both `no_std`, pure Rust). Replaced FNV-1a with Blake3 for content hashing. **Bootstrap identity is now hardware-backed**: Ed25519 public key compiled in from `bootstrap_pubkey.bin` (extracted from YubiKey via `sign-elf --export-pubkey`). No secret key in kernel memory — `BOOTSTRAP_SECRET_KEY` stays zeroed. `handle_obj_put` creates unsigned objects; `handle_obj_get` verifies signature before returning content (unsigned objects allowed). `SignedBinaryVerifier` with ARCSIG signature trailer format (64-byte Ed25519 sig + 8-byte magic). `load_boot_modules` requires signed ELFs. Host-side `tools/sign-elf/` signs via YubiKey OpenPGP (default) or seed (CI fallback). Makefile signs hello.elf, key-store-service.elf, and fs-service.elf during ISO/image build. 214 tests total.
...
- 214 unit tests run on host macOS target (`x86_64-apple-darwin`), including 12 portable AArch64 logic tests, 50 identity/ObjectStore/crypto tests, 7 signed ELF verifier tests, 1 PerCpu field offset test
