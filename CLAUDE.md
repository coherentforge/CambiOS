...
| `arch/x86_64/ioapic.rs` | I/O APIC device IRQ routing | Done — `gic::enable_spi` wired via `handle_wait_irq` in `src/syscalls/dispatcher.rs` (demand-driven on first wait; GIC default level trigger matches PCI). `set_spi_trigger` exists for future edge-mode drivers. No current driver consumes device IRQs — virtio-blk / virtio-net / virtio-input all poll. | Phase R-3 — PLIC driver (`arch/riscv64/plic.rs`); `claim()`/`complete()` in trap handler when `scause` indicates external interrupt |
