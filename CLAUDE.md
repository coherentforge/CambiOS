...
| **Policy / `on_syscall` / interceptor decisions** | [ADR-006](docs/adr/006-policy-service.md), [ADR-002](docs/adr/002-three-layer-enforcement-pipeline.md), [ADR-003 § Divergence](docs/adr/003-content-addressed-storage-and-identity.md#divergence) (kernel-side `dyn` → enum-dispatch precedent — interceptor migration is queued behind it) | `src/ipc/interceptor.rs`, `src/ipc/mod.rs` (look for `// VERIFICATION DEBT:` markers on the two interceptor field sites) |
...
| **Graphics / compositor / GUI / GPU driver** | [ADR-011](docs/adr/011-graphics-architecture-and-scaling.md) (stack design), [ADR-014](docs/adr/014-compositor-scanout-driver-protocol.md) (compositor ↔ scanout-driver protocol) | [ADR-005](docs/adr/005-ipc-primitives-control-and-bulk.md) (channels are the surface-buffer transport); compositor scaffold landed as Phase Scanout-1 (`user/compositor`, headless mode), scanout-driver implementations (virtio-gpu, Intel, Limine-fb) land Scanout-2 onward per ADR-014 phased plan |
