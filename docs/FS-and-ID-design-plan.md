...
- Boot path: key-store-service is a signed boot module like fs-service. It starts before fs-service so that fs-service's `ObjPutSigned` calls can already route to the vault. fs-service's `depends_on = ["key-store-service"]` in manifest.toml encodes the order (ADR-018).
