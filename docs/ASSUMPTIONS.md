...
| `MAX_EXTENTS_PER_INODE` | 16 | [cambios-abi/src/lib.rs](../cambios-abi/src/lib.rs) | Per-inode contiguous-extent slot count for the POSIX file backend ([ADR-029 § Decision 1](adr/029-posix-file-storage-model.md)). On-disk inode header reserves 192 bytes for extents (16 × 12 bytes packed). Mirrors the `MAX_CONTENT_BYTES_ON_DISK` precedent above: peer on-disk-format slot count whose growth requires a format-version bump rather than a routine resize. The 16-extent cap creates a real failure mode (`EFRAGMENTED`) on aged disks; v1 ships without a defragmenter per ADR-029 § Open Questions. | Growing this requires a new inode-format version bump. Trigger: the first widespread `EFRAGMENTED` failure in a production workload after the POSIX backend lands (Phase 4 follow-on), or proactively before v1 ships if disk-stress testing surfaces it. |
| `MAX_INODE_ACL_ENTRIES` | 16 | [cambios-abi/src/lib.rs](../cambios-abi/src/lib.rs) | Per-inode inline ACL slot count for the POSIX file backend ([ADR-029 § Decision 1](adr/029-posix-file-storage-model.md)). On-disk inode header reserves 704 bytes for ACL (16 × 44 bytes packed). Same SCAFFOLDING shape as `MAX_OBJECT_CAPS = 8` above (peer on-disk inline-ACL cap). Multi-Principal workloads route through ADR-027 cluster delegation per ADR-029 § Decision 3 rather than stuffing more entries into the inode. | Growing this requires a new inode-format version bump. Trigger: a workload appears where cluster delegation is structurally wrong for the access pattern per ADR-029 § Open Questions ("ACL extension blocks"). |
| `JOURNAL_BYTES` (POSIX backend) | 16 MiB | [src/fs/posix/mod.rs](../src/fs/posix/mod.rs) | Fixed-size circular metadata journal for the POSIX backend per [ADR-029 § Decision 5](adr/029-posix-file-storage-model.md). At per-record sizes from § Decision 5 holds ~1M records; checkpoint cadence (every 4 KiB or 100 ticks, whichever first) bounds steady-state usage well below capacity. On-disk cost: 16 MiB of disk per partition. | Observed flush-stall behavior under sustained metadata churn per ADR-029 § Open Questions ("Journal compaction strategy"). |
| `MAX_INODES_ON_DISK` (POSIX backend) | 4_194_304 | [src/fs/posix/mod.rs](../src/fs/posix/mod.rs) | Ceiling on declared inode capacity for the POSIX backend per [ADR-029 § Architecture](adr/029-posix-file-storage-model.md). Sized for Win-compat endgame: tens of thousands of files per app × tens of apps = low millions per personal machine. 4M = ~4× headroom over highest realistic single-machine workload. On-disk cost at full capacity: 32 GiB inode region (well above any v1 deployment). Exists to make mount-time iteration provably bounded for the verifier. | A deployment legitimately wants >4M inodes; bound rises or per-tier policy carries a tier-specific ceiling (analogous to [ADR-008](adr/008-boot-time-sized-object-tables.md) / [ADR-009](adr/009-purpose-tiers-scope.md)). |
| `MAX_BLOCKS_ON_DISK` (POSIX backend) | 1_073_741_824 | [src/fs/posix/mod.rs](../src/fs/posix/mod.rs) | Ceiling on declared block-region capacity for the POSIX backend per [ADR-029 § Architecture](adr/029-posix-file-storage-model.md). 1B blocks × 4 KiB = 4 TiB max partition. Bitmap region at full capacity is 1 GiB (a fraction of the data region). Aged disks hit `MAX_EXTENTS_PER_INODE` long before this. | A v1+ deployment wants >4 TiB per partition; tiered/sparse bitmap representations land first rather than growing the flat ceiling. |
| `MAX_BITMAP_MUTATIONS_PER_RECORD` | 32 | [src/fs/journal.rs](../src/fs/journal.rs) | Maximum bitmap-bit flips bundled in a single `ExtentUpdate` journal record per [ADR-029 § Decision 5](adr/029-posix-file-storage-model.md). Worst-case CoW commit is 1 bitmap-set + `MAX_EXTENTS_PER_INODE = 16` bitmap-clears (replacing every existing extent in one transaction); 32 gives ~2× headroom. | A batch primitive (batch-rename, batch-unlink, batch-import) requires more mutations per transaction than the current per-record cap. |
| `MAX_DIRENTS_PER_RECORD` | 8 | [src/fs/journal.rs](../src/fs/journal.rs) | Maximum directory entries packed into a single `DirectoryEntry` journal record. v1 mkdir/unlink/rename touch at most a few entries; 8 gives ~3× headroom over typical and bounds per-record size. | A bulk-directory primitive (batch-import, directory-tree clone) requires more entries per record. |
...
| `MAX_TRUSTED_KEYS` | 4 | [src/loader/mod.rs:258](../src/loader/mod.rs#L258) | Bootstrap + a few rotation keys. | First time we have CI builder + your YubiKey + backup key + rotation key, we've used the budget with zero room for new signers. Coming up faster than the other PKI items because CI signing is in the early v1 path. |
| `MAX_USER_BUFFER` | 4096 | [src/syscalls/dispatcher.rs:40](../src/syscalls/dispatcher.rs#L40) | Single-syscall arg buffer cap; bounds copy_from_user / copy_to_user. | A user-space service that needs to read or write > 4 KB in one syscall and gets a confusing failure at exactly the boundary. Channels (ADR-005) are the long-term answer for bulk; until then this needs to grow on demand. |
...
| `MAX_RAM_EXTENTS` | 16 | [src/memory/frame_allocator.rs](../src/memory/frame_allocator.rs) | USABLE-RAM extent table populated by `add_region` and consumed by `is_ram_overlap` (the `SYS_MAP_MMIO` security check). Typical x86 PC: 2-3 extents (low DOS area, low memory, high memory above the PCI hole). NUMA workstation: ~8. 16 covers v1-endgame with headroom. Memory cost: 16 × 16 B = 256 B in the FrameAllocator. | A real workload's Limine memmap reports > 12 USABLE extents (75% utilization); extents 13+ silently fall off today's table (false-negative on the MMIO check, not memory corruption — the bitmap-tracked frame allocator still refuses to hand out those frames). |
| `ECAM_MAP_BYTES` | 1 MiB | [src/pci/mod.rs](../src/pci/mod.rs) | aarch64 + riscv64 ECAM window mapped at boot (bus 0 only = 256 BDFs × 4 KiB). Keeps aarch64 init cost at 2 intermediate page-table frames (L2 + L3); the full 256 MiB advertised by QEMU virt would need 128 L3 tables with no discovery benefit today. | A multi-bus topology boots (second PCIe bridge visible, or an MCFG/DTB parser that surfaces more than one `bus_start..bus_end` range). |
| `ECAM_PHYS_AARCH64` | `0x40_1000_0000` | [src/microkernel/main.rs](../src/microkernel/main.rs) | QEMU `virt` aarch64 high PCIe ECAM base. Matches the existing `GICD_PHYS` / `GICR_PHYS` precedent — platform-fixed by QEMU's `extended_memmap[VIRT_HIGH_PCIE_ECAM]`. | An MCFG/DTB parser lands (ADR-021 Phase C) or a non-QEMU aarch64 target boots. |
| `ECAM_SIZE_AARCH64` | 256 MiB | [src/microkernel/main.rs](../src/microkernel/main.rs) | Size QEMU advertises for the high ECAM window. `pci::init_ecam` caps the actual mapping at `ECAM_MAP_BYTES`; passing the full advertised size keeps a future multi-bus change a single-line edit. | Same trigger as `ECAM_PHYS_AARCH64`. |
| `MAX_MEMORY_REGIONS` (BootInfo) | 128 | [src/boot/mod.rs:51](../src/boot/mod.rs#L51) | Boot-protocol-agnostic kernel-owned memory map size (populated by `boot::limine::populate`). QEMU reports ~10-15; bare-metal firmwares ≤ several dozen. 128 covers all realistic firmwares with margin. Memory cost: 128 × ~24 B ≈ 3 KiB in .bss. | Real firmware reports >128 entries (would indicate badly-fragmented memory worth investigating regardless). Landed 2026-04-14 as part of the [ADR-011](adr/011-graphics-architecture-and-scaling.md) Limine→camBIOS abstraction. |
| `MAX_FRAMEBUFFERS` (BootInfo) | 8 | [src/boot/mod.rs:60](../src/boot/mod.rs#L60) | Kernel-owned framebuffer list; boot adapter populates. Matches [ADR-011](adr/011-graphics-architecture-and-scaling.md) multi-monitor target of 3+ displays with ~2× headroom. Memory cost: 8 × ~40 B ≈ 320 B. | Workstation deployments with >8 active displays (uncommon even for pro rigs). |
| `MAX_BOOT_MODULES` (BootInfo) | 16 | [src/boot/mod.rs:69](../src/boot/mod.rs#L69) | Mirrors the runtime `BootModuleRegistry::MAX_MODULES` used by the spawn syscall. BootInfo holds the boot-protocol view; registry holds the spawnable view. | Tied to boot-module count growth — see `MAX_MODULES` row. |
| `MAX_MODULE_NAME_LEN` (BootInfo) | 64 | [src/boot/mod.rs:72](../src/boot/mod.rs#L72) | Fixed-size module-name buffer in BootInfo. Must equal [`crate::boot_modules::MAX_NAME_LEN`] so strip-then-register round-trips cleanly. | A boot module path > 64 chars after stripping (see `MAX_NAME_LEN` row). |
| `FB_DESC_SIZE` (SYS_MAP_FRAMEBUFFER wire format) | 32 | [src/syscalls/dispatcher.rs handle_map_framebuffer](../src/syscalls/dispatcher.rs) | ARCHITECTURAL: v1 framebuffer descriptor ABI — `{vaddr:u64, width:u32, height:u32, pitch:u32, bpp:u16, red/green/blue mask sizes+shifts, reserved}`. Mirrored by `libsys::FramebufferDescriptor`. Changing this is a new syscall + capability, not a size bump. See [ADR-011](adr/011-graphics-architecture-and-scaling.md). | Intentionally never — this is the boundary every future compositor / GPU driver speaks. |
| `MAX_CHANNELS` | 256 | [src/ipc/channel.rs:40](../src/ipc/channel.rs#L40) | Bounded channel table for verification. Sized for v1 endgame multi-monitor compositor ([ADR-011](adr/011-graphics-architecture-and-scaling.md)): ~6 scanout channels (3 displays × front/back) + ~30 window surfaces + ~10 GPU command/memory channels + non-GUI services (~14 from current boot modules) = ~60 active. 4× headroom per CLAUDE.md Convention 8. Memory cost: 256 × ~272 B ≈ 70 KiB (was ~40 KiB pre-[ADR-030](adr/030-stream-cap-variant.md); the optional `stream_cap_shape` + `stream_state` fields added ~112 B per record). | Bumped 2026-04-13 from 64 in the Phase GUI-0 prep pass. Next revisit: multi-monitor + many-client graphics workloads exceeding 60 active channels, or a service that legitimately needs dozens of simultaneous data paths. |
| `MAX_CHANNEL_PAGES` | 65536 (256 MiB) | [src/ipc/channel.rs:56](../src/ipc/channel.rs#L56) | Soft cap on channel size in pages. Sized for v1 endgame graphics ([ADR-011](adr/011-graphics-architecture-and-scaling.md)): a 4K display at 2× Retina backing scale = 8K backing store = 128 MiB @ 32bpp, 256 MiB @ 64bpp HDR. Single-channel ceiling accommodates one such full-screen window surface; multi-monitor workloads use multiple channels (one per display scanout, one per window surface). Ceiling is soft (not always-allocated). | Bumped 2026-04-13 from 4096 in the Phase GUI-0 prep pass. Next revisit: HDR + backing scale beyond 2× on 5K/8K displays pushes single-surface size past 256 MiB, at which point the tier-aware policy service (Phase 3.4) and a `LargeChannel` capability should gate these allocations before the ceiling rises further. |
| `MIN_CHANNEL_PAGES` | 1 (4 KiB) | [src/ipc/channel.rs:62](../src/ipc/channel.rs#L62) | Minimum channel size. Channels smaller than a page would defeat the purpose — the 256-byte control IPC already covers small messages. |
| `QUIESCE_TIMEOUT_TICKS` | 10 (100ms @ 100Hz) | [src/ipc/channel.rs:84](../src/ipc/channel.rs#L84) | Per-channel quiesce timeout per [ADR-027 § Decision 3](adr/027-service-clusters.md). Bounds `SYS_CHANNEL_REVOKE` and the `revoke_all_for_process` sweep on `SYS_EXIT` when the peer is mid-blit on another CPU. ≥ 1 timer tick (10ms at 100Hz) so involuntary preemption catches the peer at next ISR; 10× that for slow yield paths under load. < human-perceptible latency (~150-200ms application response threshold) so a clean client exit's compositor #PF resolution is invisible. | A real workload shows quiesce regularly hitting timeout (audit drop counter rises, or kill-task fires for cooperative peers). At that point either the ceiling rises with workload-anchored rationale, or trust-tier-1 services get a cooperative-only path skipping the timeout. |
| `MAX_QUIESCE_WAKE` | 8 | [src/scheduler/mod.rs](../src/scheduler/mod.rs) `wake_quiesce` | Stack-resident enqueue-buffer cap on `Scheduler::wake_quiesce`. v1's 1:1 process↔task linkage means at most 1 peer task can be parked in `Blocked(ChannelQuiesceWait(channel_id))` for a given channel, so the realistic count is 1; 8 leaves slack for a future N:1 task model (multi-threaded user processes parking multiple tasks against the same channel during cooperative quiesce). Bounded so the wake path never allocates from `complete_teardown`. Memory cost: 8 × `(TaskId, usize)` ≈ 96 B on the stack frame of one wake call — negligible. | Process↔task linkage becomes 1:N (`Process` owns multiple `Task`s) AND a real workload parks more than one of those tasks on the same `ChannelQuiesceWait`. At that point the cap moves with measured peak parked-tasks-per-channel + headroom. |
| `MAX_CLUSTERS` | 32 | [src/ipc/cluster.rs:42](../src/ipc/cluster.rs#L42) | Cluster table cap per [ADR-027 § Architecture](adr/027-service-clusters.md). v1 endgame extrapolation: ~4-8 simultaneous clusters (rendering limb, storage limb, audio limb, init coordinator); 4× headroom puts the ceiling at 32. Memory cost (32 × Option<ClusterRecord>) is well under 64 KiB, negligible vs the channel table (~40 KiB). | A v1.5+ workload wants per-user-session clusters or hot-pluggable cluster instantiation that pushes past 32 live clusters. |
| `MAX_CLUSTER_MEMBERS` | 32 | [src/ipc/cluster.rs:51](../src/ipc/cluster.rs#L51) | Per-cluster member-list cap per [ADR-027 § Architecture](adr/027-service-clusters.md). v1 endgame: rendering limb has 3 members today (compositor + scanout + input); storage ~2; audio TBD. 4× headroom = 32. | A single cluster type exceeds ~8 distinct roles - e.g. multi-display rendering limb with one scanout-driver per attached display. Replacement bound tracks the largest single cluster's member count, not the sum across clusters. |
| `MAX_CLUSTER_CHANNELS` | 32 | [src/ipc/cluster.rs:62](../src/ipc/cluster.rs#L62) | Per-cluster channel-list cap per [ADR-027 § Architecture](adr/027-service-clusters.md). v1 endgame: rendering limb composes ~6 channels (scanout + per-window surfaces + input event queue); 4× headroom = 32. | A cluster type exceeds ~8 internal channels (e.g. scanout limb owning one channel per display). Cap relaxation needs a verification pass on cluster-revoke timing under MAX_CLUSTER_CHANNELS × per-channel-revoke cost. |
...
| `RECENT_EXITS_RING_SIZE` | 64 | [src/process.rs](../src/process.rs) | Bounded circular buffer on `ProcessTable` for `SYS_GET_PROCESS_PRINCIPAL` lookups when the target process has already exited (ADR-023). At ~10 process exits/min steady state and audit consumers reading at ≥1/sec, 64 entries covers ~6 minutes of exit history — comfortably more than the audit ring's ~1s buffer. 25%-utilization rule: typical workload uses ~16 entries. Memory cost: 64 × 40 B ≈ 2.5 KiB on `ProcessTable` (stored inline). | Kernelvisor stress test shows >64 distinct exits within any 10-second consumer-lag window, or a per-Principal resolution table replaces the ring entirely. |
...
|---|---|---|---|
| Control IPC payload | 256 bytes | [src/ipc/mod.rs:98](../src/ipc/mod.rs#L98) | Fixed for verification: kernel reads every byte of every control-IPC message. Bulk data takes a separate path (channels — see [ADR-005](adr/005-ipc-primitives-control-and-bulk.md)). |
| `NUM_PRIORITY_BANDS` | 4 | [src/scheduler/mod.rs:135](../src/scheduler/mod.rs#L135) | Idle / Low / Normal / High+Critical — the priority taxonomy. 4 bands is the design, not a tuning choice. |
...
| Lock hierarchy depth | 10 | n/a | The ten-lock hierarchy in [CLAUDE.md § Lock Ordering](../CLAUDE.md#lock-ordering). Bumped from 9 → 10 in 2026-05-07 when ADR-027 inserted `CLUSTER_MANAGER` at position 5 (cascade renumber for everything below). Adding a lock is a deliberate architectural decision, not a number bump. |
| `MEMBER_BYTES` (SYS_CLUSTER_CREATE wire) | 36 | [src/syscalls/dispatcher.rs](../src/syscalls/dispatcher.rs) | ARCHITECTURAL: per-member spec on the SYS_CLUSTER_CREATE syscall boundary — `{ principal: [u8; 32], role: u32 }` packed = 36 bytes. ADR-027 cluster manifest wire format. Changing this is a new syscall, not a size bump. |
| `CLUSTER_INFO_BYTES` | 28 | [src/syscalls/dispatcher.rs](../src/syscalls/dispatcher.rs) | ARCHITECTURAL: SYS_CLUSTER_INFO output descriptor — `[state:u32][policy:u32][creator_pid_slot:u32][joined_members:u32][active_channels:u32][created_at_tick:u64]` = 28 bytes. ADR-027. Changing this is a new syscall, not a size bump. |
...
| `ARCOBJ_MAGIC` | `"ARCOBJ00"` | [src/fs/disk.rs:28](../src/fs/disk.rs#L28) | Persistent ObjectStore superblock magic ([ADR-010](adr/010-persistent-object-store-on-disk-format.md)). Changing it is a new format family, not a version bump. Readers that see a different magic treat the disk as corrupt. |
| `ARCOREC_MAGIC_OCCUPIED` | `"ARCOREC1"` | [src/fs/disk.rs:31](../src/fs/disk.rs#L31) | Record-header magic for an occupied slot. Commit point of the write protocol (ADR-010). Absence = free. The `1` at the end distinguishes from a future `ARCOREC2` if record layout changes. |
| `FORMAT_VERSION` | 1 | [src/fs/disk.rs:35](../src/fs/disk.rs#L35) | Persistent ObjectStore on-disk format version. Mount rejects unknown versions. Version 2 lands when ML-DSA signatures or multi-block content records require a layout change. |
...
| `GDT_ENTRIES` | 7 | [src/arch/x86_64/gdt.rs:40](../src/arch/x86_64/gdt.rs#L40) | Null + kernel CS + kernel SS + user SS + user CS + TSS low + TSS high. SYSRET requires this exact layout. |
| `IST_STACK_SIZE` | 4 KiB | [src/interrupts/mod.rs:438](../src/interrupts/mod.rs#L438) | Double-fault handler dedicated stack. The double-fault handler is small; doesn't need more. |
...
| `SIGNATURE_TRAILER_SIZE` | 72 (64 + 8) | [src/loader/mod.rs:232](../src/loader/mod.rs#L232) | Ed25519 signature (64 B) + ARCSIG magic (8 B). Fixed by the on-disk format. |
| `BLOCK_SIZE` | 4096 | [src/fs/block.rs:30](../src/fs/block.rs#L30) | On-disk sector size for the persistent ObjectStore ([ADR-010](adr/010-persistent-object-store-on-disk-format.md)). Matches x86_64 page size and is a multiple of every common physical sector size (512 B and 4K-native). All `BlockDevice` I/O is sized at this boundary regardless of whether the underlying hardware reports 512-byte or 4K-native sectors. |
...
These three rows capture the default `TableSizingPolicy` per deployment tier introduced by [ADR-008](adr/008-boot-time-sized-object-tables.md) and [ADR-009](adr/009-purpose-tiers-scope.md). Each policy is a 5-field struct — `min_slots`, `max_slots`, `ram_budget_ppm`, `ram_budget_floor`, `ram_budget_ceiling` — that drives the boot-time `num_slots` computation in [`config::num_slots_from`](../src/config/tier.rs). They are TUNING because the defaults are starting points chosen from estimated workload density; the right values depend on what each tier's deployments actually run, and will shift as real workload data arrives. The kernel binary is identical across tiers per ADR-009; what differs is which policy is selected by the `CAMBIOS_TIER` build environment variable (default: `tier3`), wired in via [`build.rs`](build.rs).
...
2. Pick the smallest value where the v1 estimate is **≤ 25% of the bound** AND the memory cost is still comfortable. ~4× headroom absorbs surprising workloads or unplanned consumers (a new audit channel, a second policy cache).
3. Record the math in the bound's row below — "Why this number" should show v1 workload estimate + memory cost, not just "big enough."
