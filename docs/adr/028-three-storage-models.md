...
- **Supersedes:** [ADR-015](015-storage-tiers-and-commitment-ladder.md) (the commitment-ladder framing; the three-storage-models reframe is incompatible with promotion-shaped storage tiers). [ADR-003](003-content-addressed-storage-and-identity.md) gets a Divergence appendix when this ADR ratifies (narrowing the "files are not bytes-at-a-path" claim without altering the CambiObject model).
