...
- The reaper frees the slot **cross-CPU** (the child may have migrated) via `PER_CPU_SCHEDULER[child_cpu]` resolved through `TASK_CPU_MAP`; `WaitTask` must likewise resolve the child cross-CPU for both its pre-block check and post-yield read.
...
- **Phase A — the deferral:** per-CPU reaper tasks + `PER_CPU_RECLAIM_QUEUE`; defer `{ root + intermediates + kernel stack }` (not the slot). Closes the x86_64 + riscv64 triple-fault and the kernel-stack leak. Adds the **release-build active-root guard** (§3), the **enqueue-once** state gate, and the **peer-walk `FRAME_ALLOCATOR` audit** (occupancy class 2). Uses the lock-isolated per-CPU queue (placement signed off).
- **Phase B — the slot leak (HV1):** TaskId generation + `TaskExitRing` + `WaitTask` rewrite + H5 co-fix + cross-CPU slot reclaim. Extends the Phase A reaper; no tear-down.
