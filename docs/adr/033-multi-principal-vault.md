...
- **Current (2026-05-29):** Phase 1C implementation complete. `user/key-store-service` carries the full `Vault` data model (`Vault { bootstrap_aid, entries: [Option<VaultEntry>; 16], context_map: [Option<ContextMapEntry>; 8] }`), `Vault::authorize` trust-boundary helper, and all three IPC primitives — `CMD_VAULT_BIND_FOR_SPAWN` (10), `CMD_VAULT_SIGN_WITH` (11), `CMD_VAULT_DECRYPT_WITH` (12) — over endpoint 17 via the existing `recv_verified` gate. Wire codec in `user/libsys/src/vault.rs`. Backend routing through the active `PivBackend` (SwPivBackend under `--features dev-piv`, InertPivBackend otherwise; CcidPivBackend lands at Stream B B-ix without touching the vault layer). v1 vault is single-bootstrap-entry; `context_map` starts empty and `bind_for_spawn` falls back to the bootstrap AID with a `[VAULT][FALLBACK]` log line on context miss (Convention 9 trigger pinned at `log_vault_fallback` for migration to a userspace audit-emit primitive when a second consumer surfaces). Dispatch arms wired but dormant on the smoke path — first real caller integrates separately (fde-mount FDE-unlock migration to `vault_decrypt_with` is the leading candidate per § Positive Consequences). Commits: 71c85a5 (data model + `Vault::authorize` + cfg-gated init), e31f9e3 (`bind_for_spawn`), 127073e (`sign_with` + `decrypt_with`).

- **Beyond:** Phases 2 (biometric recovery), 3 (social quorum), and 4 (DID encoding) extend the vault per identity.md's roadmap. The data model above accommodates each addition without structural change. Out-of-scope items in this ADR (on-disk encoding, CRDT sync, key rotation, per-context KDF plurality, `CreateProcess` cap gating, duress mode) remain deferred with their original revisit triggers.
