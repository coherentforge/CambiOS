...
- A channel ([ADR-005](005-ipc-primitives-control-and-bulk.md)) needs to be force-closed because one of its peers is misbehaving; the kernel must atomically unmap the shared memory from both processes and revoke the underlying capability.
...
2. **Audit telemetry channel** — a kernel-maintained event stream of capability-relevant operations (grants, denies, revokes, channel creates, anomaly hooks), exposed to user-space subscribers (initially the policy service, eventually the AI watcher) via the bulk-data channel mechanism from [ADR-005](005-ipc-primitives-control-and-bulk.md).

These two primitives are the substrate that makes [ADR-006](006-policy-service.md)'s policy service real. Without them, the policy service can answer "yes or no" to grant decisions but cannot intervene in already-granted authority and cannot learn from history. With them, the policy service can decide to revoke based on observed patterns, the AI watcher can subscribe to the event stream and produce recommendations, and the user retains the ability to inspect or override any of it.
...
3. **The bootstrap Principal** (and, after [ADR-006](006-policy-service.md)'s policy service exists, anyone holding the policy service's revoke endpoint capability). This is the "policy service can intervene" path. The policy service holds a special capability that lets it revoke any other capability in the system, granted at boot time as part of the policy service's bootstrap manifest.
...
With revocation, the policy service from [ADR-006](006-policy-service.md) becomes capable of **intervention**, not just decision. Concretely:
...
The audit telemetry channel uses [ADR-005](005-ipc-primitives-control-and-bulk.md)'s data channel primitive. Specifically:
...
8. **Update the policy service** ([ADR-006](006-policy-service.md)) to attach to the audit channel and start consuming. Initially the policy service just logs to its own console — no decisions based on audit events yet.
...
The order is deliberate: revocation comes first because it has no dependencies on user-space code. Telemetry follows because it depends on [ADR-005](005-ipc-primitives-control-and-bulk.md)'s channel mechanism, which is more invasive. The two together complete the substrate that [ADR-006](006-policy-service.md)'s policy service needs to be useful.

## Cross-References

- **[ADR-000](000-zta-and-cap.md)** — Capability foundations (revocation extends the existing model)
...
[ADR-026](026-identity-transcription-at-the-kernel-ring.md) framing (2026-05-02) refines three properties of this ADR's mechanism without changing what was decided. ADR-026 is the kernel-side architectural codification of the multi-Principal vault, the transcription invariant, and per-Principal AI containment; the items below are the points where that framing reaches into ADR-007's revocation + audit machinery.

4. **Containment is per-Principal, not per-process.** This ADR's "How These Two Primitives Combine" loop describes the AI watcher building behavioral baselines and the policy service revoking specific holders' caps when anomalies surface. ADR-026 § 3 sharpens the unit of containment: under the multi-Principal vault model in [identity.md § The Vault](../identity.md#the-vault), one human runs many Principals concurrently across many processes. AI baselines and policy responses key on the **Principal**, not on the process — when `Principal_X` exhibits anomalous behavior, narrowing applies to every process bound to `Principal_X` (which may be one process or several), and no process bound to `Principal_Y` is affected even if `Principal_X` and `Principal_Y` belong to the same human. The kernel-side mechanism is unchanged: `revoke_all_for_process` is the per-process exit cleanup; per-Principal narrowing is the policy service issuing one or more `SYS_REVOKE_CAPABILITY` calls keyed by holders' bound Principals. The framing — *blast radius is a context, not a person* — is what ADR-026 codifies, and what makes "AI watches and contains" a usable architecture rather than a brittle one.

5. **The revocation primitive specified here is the *internal-cap* form.** ADR-026 § 2 (capability shape duality) names the boundary explicitly. `SYS_REVOKE_CAPABILITY` and `CapabilityManager::revoke()` operate on the kernel's internal cap form — a `(endpoint, rights)` table entry under `CAPABILITY_MANAGER`. Synchronous, kernel-led, push-based; correct for caps the kernel directly owns, and the invariants in this ADR's "What revocation means" hold for that form. They do **not** cover the *external-cap* form: caps that have been emitted across a trust boundary (stored alongside CambiObjects per [ADR-010](010-persistent-object-store-on-disk-format.md), included in boot manifests per [ADR-018](018-init-process-and-boot-manifest.md), handed across processes via signed delegation, persisted across reboots). External caps need an asynchronous, holder-led, pull-based revocation: the issuer maintains a non-revocation accumulator (a small constant-size cryptographic structure published as a CambiObject); cap holders carry witnesses against the accumulator; verification at use checks witness-against-current-accumulator membership; revocation is an accumulator update — holders converge asynchronously by refreshing their witnesses, and revoked holders cannot generate a new witness because their cap was excluded from the new accumulator. **A future amendment to this ADR will specify the witness-and-accumulator wire/disk format**; this Divergence entry only names the slot. Until that amendment lands, persistent caps have no revocation path beyond "delete the object." [identity.md § Capability Revocation (Tiered by Cap Shape)](../identity.md#capability-revocation-tiered-by-cap-shape) carries the user-facing framing for both forms.

6. **Bootstrap-Principal authority on `SYS_REVOKE_CAPABILITY` is a transcription-invariant vestige, slated for migration.** The Phase 3.1 implementation in [src/ipc/capability.rs:720](../../src/ipc/capability.rs#L720) (`CapabilityManager::revoke`) gates the syscall by checking `revoker_principal != bootstrap`. Per ADR-026 § 1 (transcription invariant), this is a kernel hot-path branch on a Principal value — exactly the kind of identity *interpretation* that [ADR-023](023-audit-consumer-capability.md) has already started migrating elsewhere by replacing equality checks with capability checks. The migration target for this site, when Phase 3.4's policy service lands as the mediator (see this ADR's "Who can revoke" — the third authority path), is a system capability (e.g., `CapabilityKind::RevokeCapability`) replacing the Principal-equality test, with the cap granted to the policy service at boot via the manifest. Documenting the vestige here means a future migrator can cite this Divergence + ADR-026 § 1 when collapsing the check rather than re-deriving the rationale. The migration is not a behavior change — it converts an interpretation site to a transcription-compatible site without altering who can revoke in practice.

[ADR-027](027-service-clusters.md) Phase 1 quiesce protocol delivery (2026-05-05) refines this ADR's channel-mapping-teardown mechanism for the v1 fix of the open compositor #PF on clean client exit (HN-blocker, first observed 2026-04-25). The capability-revocation mechanism, the channel record state machine, and the audit emission paths are unchanged; the items below alter how the **physical mapping** that backed a revoked channel is torn down from the peer's address space.

7. **Channel revoke replaces the peer's RO mapping with a tombstone, instead of unmapping it.** The original ADR-007 § "How revocation interacts with channels" specifies "Look up the channel's page table mappings in that holder → Unmap the pages → Issue TLB shootdown … the holder's next access through stale references faults." For RO peer mappings (compositor reading a libgui-client surface; udp-stack reading a virtio-net rx ring) the v1 implementation **replaces the unmap step with a remap to a kernel-owned shared RO zero page** (the *tombstone*). The PTE swap + TLB shootdown still happen; what changes is what the peer's TLB now resolves the vaddr to.

   *Why the unmap-then-fault contract didn't close the HN bug.* Quiesce-then-unmap (the [ADR-027 § Decision 3](027-service-clusters.md) protocol delivered by commits introducing `ChannelState::Revoking`, `Scheduler::arm_quiesce`, and `BlockReason::ChannelQuiesceWait`) ensures the peer task is parked off-CPU at the moment of unmap. But the peer's user-space RIP is preserved verbatim across park-and-wake — the saved register state is not rewindable to a "safe point" without an in-process fault handler primitive (path iii in the planning notes; deferred). After wake, `iretq` restores the saved context and the very next user-space instruction is a read from the now-unmapped page, which faults exactly as it did pre-quiesce. The post-wake fault is endemic to unmap-on-revoke whenever the peer holds a stale user-space pointer; the quiesce protocol shifts *when* the fault happens (after wake instead of during unmap) but does not eliminate it.

   *Why tombstone closes it.* The peer's RIP is preserved across the swap; the next read returns zeros from the tombstone instead of faulting. Compositor user-space code paths that haven't yet learned the channel is gone (e.g., `blit_surface_to_scanout`'s pixel loop) consume zeros until the userspace half of the fix runs (a `SYS_CHANNEL_INFO` check at the top of the relevant blit / read path; observes `ChannelState::Revoked`; drops the dead window from `WindowTable`). Without that userspace check, tombstone prevents the fault but the peer keeps reading zeros from the dead mapping — a visual artifact, not a crash. With it, the peer transitions to "no longer references the dead surface" within one frame.

   *Where the tombstone lives.* One global RO 4 KiB page, allocated at kernel init via `FrameAllocator::allocate_contiguous(1)` and never freed. All revoked RO peer mappings point to it; original channel frames are freed back to the allocator on revoke. The fact "tombstone is RO, all-zero, never freed, single page" is a kernel-static assertion verifier-friendly to encode.

   *Compatibility with the quiesce protocol.* The quiesce machinery is unchanged and remains useful for SMP races where the peer holds an RW mapping (entry 8 below) — quiesce ensures the peer's CPU is not in the middle of a write when the kernel issues the TLB shootdown for the swap. For RO peer mappings under tombstone, quiesce becomes best-effort / advisory in v1: the swap is safe to perform regardless of whether the peer is currently reading (TLB shootdown invalidates stale entries; subsequent reads hit the tombstone). `arm_quiesce` is still invoked by the revoke handler so the protocol shape carries forward to ADR-027 cluster revoke without retrofit.

8. **RW peer mappings keep the unmap behavior; tombstone is RO-only in v1.** Tombstone-as-RW would mean arbitrary peers writing to a single kernel-shared page — a data-corruption vector and a cross-process leak channel between writers that share no other authority. Resolving this requires per-peer scratch pages (allocated at attach time, freed at revoke), which is materially larger than the v1 fix. RW peer mappings (Producer-role channel's peer is RW; Bidirectional channel both sides RW) therefore keep the unmap-and-shootdown behavior, and the peer faults on subsequent writes.

   The asymmetry covers v1 cleanly because the HN-blocker case is RO from the compositor's side: libgui clients create surface channels with `CHANNEL_ROLE_CONSUMER` (creator-reads), making the compositor (creator) RO and the client (peer) RW. When the client exits, the compositor's mapping is the RO direction → tombstone closes the fault. The other directions (compositor writing scanout where scanout-driver dies; udp-stack writing virtio-net rx where virtio-net dies) leave RW peer mappings and the peer faults; these are catastrophic-state cases — when the consumer of an RW channel dies, the writer has no destination and the kill-via-fault outcome is acceptable v1 behavior.

   **Replace when:** a workload requires hot-restart of an RW-channel consumer where the writer must survive the consumer's death. Likely shape is per-peer scratch pages allocated at `SYS_CHANNEL_ATTACH` time, freed at revoke, with writes silently sinking. Defer until the first such workload surfaces.
