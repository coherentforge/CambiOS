...
4. **Defense in depth on every message.** Because every byte goes through the kernel, every byte gets checked. Capability verification, interceptor policy, sender stamping, payload bounds, endpoint bounds — all of [ADR-002](002-three-layer-enforcement-pipeline.md)'s pipeline runs on every send. This is the right tradeoff for control messages, where security matters more than throughput.
...
The capability is held in the same `ProcessCapabilities` table the existing endpoint capabilities live in, so the kernel's revocation, delegation, and policy checks all extend naturally — see [ADR-007](007-capability-revocation-and-telemetry.md) for revocation specifics.
...
The primary notification mechanism is **control IPC**: when a producer has written a batch of data into a channel ring buffer, it sends a small message through its existing 256-byte path saying "X bytes available." The consumer wakes (via the existing IPC blocking primitives — see [SCHEDULER.md § Blocking and Wake Primitives](../../src/scheduler/SCHEDULER.md#blocking-and-wake-primitives)), reads from the shared memory, and processes.
...
The TLB shootdown uses the existing IPI machinery on x86_64 ([ADR-001 § Lock Ordering](001-smp-scheduling-and-lock-hierarchy.md#lock-ordering)) and the TLBI broadcast on AArch64.

### Capability revocation interaction

Channel close and channel revoke both interact with the capability manager (which sits at lock position 4 in the hierarchy). The full revocation primitive — including how a third party initiates it and how the kernel atomically tears down the mapping — is specified in [ADR-007](007-capability-revocation-and-telemetry.md).
...
[ADR-000 § Threat Model](000-zta-and-cap.md#threat-model) lists threats and their mitigations. Channels do not weaken any of those. They add a new primitive to which the same enforcement principles apply:
...
The trade is: we get a primitive that scales to any bandwidth, in exchange for verifying a different set of invariants than the control IPC verifies. The total verification surface increases, but each subsystem's invariants stay individually tractable. This is consistent with [CLAUDE.md § Formal Verification](../../CLAUDE.md#formal-verification-non-negotiable-constraint): "pure logic separated from effects" — the channel manager's logic (allocate, map, sign, record) is pure with respect to the data flow, and the data flow is the MMU's job.
...
- **[ADR-000](000-zta-and-cap.md)** — Capability foundations (channels are capabilities)
- **[ADR-002](002-three-layer-enforcement-pipeline.md)** — Three-layer enforcement (still applies to control IPC; channels add their own setup-time check)
...
## Divergence: 2026-05-12 - Implementation Drift Catch-up

- **Trigger:** Pre-v1 review of ADR-005 surfaced seven places where the implementation has moved on since the original 2026-04-10 decision text. None of the deltas weaken the architecture; they reflect refinements ratified in subsequent ADRs ([ADR-007 § Divergence 7-9](007-capability-revocation-and-telemetry.md#divergence), [ADR-027](027-service-clusters.md)), one cosmetic decision (no separate `libchannel` crate), and the discovery that the actual first channel consumer was the rendering pipeline rather than the data plane this ADR predicted. The catalog is recorded here so future readers do not re-derive it.

- **1. `CHANNEL_MANAGER` lock position is 6, not 5.** The original Cross-References note placed the channel manager between `CAPABILITY_MANAGER(4)` and `PROCESS_TABLE(6)`. [ADR-027 § Architecture](027-service-clusters.md) inserted `CLUSTER_MANAGER` at position 5; `CHANNEL_MANAGER` is now position 6 and `PROCESS_TABLE` is position 7. Authoritative hierarchy lives in [CLAUDE.md § Lock Ordering](../../CLAUDE.md#lock-ordering) and [src/lib.rs](../../src/lib.rs). No reorder relative to `CAPABILITY_MANAGER` or `PROCESS_TABLE` - the cluster manager slotted *between* them rather than displacing channel state.

- **2. `ChannelState` has five variants, not four.** The original enum was `AwaitingAttach | Active | Revoked | Closed`. The v1 fix for the compositor #PF on clean client exit (HN-blocker, closed 2026-05-06) added `Revoking` as the transitional state used by [ADR-027 § Decision 3](027-service-clusters.md)'s quiesce protocol and [ADR-007 § Divergence 7](007-capability-revocation-and-telemetry.md#divergence) tombstone-on-revoke. Implementation in [src/ipc/channel.rs](../../src/ipc/channel.rs). The state machine remains terminal at `Revoked` / `Closed`; `Revoking` is a transient pre-tombstone gate, not a new resting state.

- **3. Three more channel-related syscalls landed.** This ADR's table listed five (28-32). The set has grown to eight, all owned by [cambios-abi/src/lib.rs](../../cambios-abi/src/lib.rs):
  - `ChannelQuiesceAck = 43` - acks the [ADR-027](027-service-clusters.md) quiesce arm so the kernel can proceed to the tombstone swap.
  - `ChannelBeginTeardown = 48` and `ChannelCompleteTeardown = 49` - two-phase teardown variants for cluster-shaped revoke, per [ADR-027 § Architecture](027-service-clusters.md). The original single-phase `ChannelClose` (sender-driven) and `ChannelRevoke` (bootstrap-Principal / third-party) remain unchanged for the existing call sites; the two-phase variants are how cluster revoke atomically tears down N channels under a single locked transition.

  The "kernel touches a channel exactly four times" framing in this ADR's Decision § 2 holds for the single-phase paths; the two-phase teardown adds two boundary events per channel for the cluster-revoke path. The substrate model is unchanged - boundary mediation, no per-byte involvement.

- **4. Revoke unmaps RW peers and tombstones RO peers.** This ADR's Threat Model row "writes after revocation fault" remains correct for RW peer mappings (Producer-role channel's peer; Bidirectional both sides). For RO peer mappings (Consumer-role channel's peer), [ADR-007 § Divergence 7-9](007-capability-revocation-and-telemetry.md#divergence) refined the contract: the kernel swaps the peer's RO PTE to point at a kernel-owned all-zero tombstone page (single global RO 4 KiB frame, never freed) rather than unmapping it. Reads return zeros; the channel record stays observable via `SYS_CHANNEL_INFO` so cooperating userspace drops the dead reference within one polling round. The no-leak invariant in this ADR's Verification Stance is preserved - original channel frames are freed back to the allocator, the peer's mapping points to the tombstone, freed frames never alias the tombstone. The asymmetry exists because tombstone-as-RW would be a shared-write data-corruption vector; per-peer RW scratch pages remain deferred until a workload requires hot-restart of an RW-channel consumer.

- **5. `SYS_CHANNEL_INFO` gained a load-bearing userspace role.** The original table entry described it as "read channel metadata (size, peers, purpose, byte counts)." Per item 4 above, cooperating userspace now polls it to observe `ChannelState::Revoked` and drop the dead reference after tombstone-on-revoke. Reference site: [user/compositor/src/windows.rs](../../user/compositor/src/windows.rs) (`channel_info` sweep in the per-frame reap pass). The metadata-read use case is unchanged; the tombstone-detection use case is additive.

- **6. No separate `libchannel` crate.** Migration Path Stage 4 promised a user-space `libchannel` crate wrapping channel handles + ring buffer protocol. What shipped instead: `channel_create` / `channel_attach` / `channel_close` / `channel_revoke` / `channel_begin_teardown` / `channel_complete_teardown` / `channel_quiesce_ack` / `channel_info` live in [user/libsys/src/lib.rs](../../user/libsys/src/lib.rs). Functional outcome is identical; the cosmetic decision was to avoid a third user-space crate when the existing libsys is the single ABI surface every user binary already depends on. The ring-buffer protocol described in Implementation Notes (head/tail in the first page, SPSC default) is now a per-consumer convention (libgui surfaces, scanout-driver pixel ring, virtio-blk request ring each tailor their own layout) rather than a shared library.

- **7. The first channel consumer was the rendering pipeline, not virtio-net ↔ udp-stack.** Migration Path Stage 5 predicted virtio-net ↔ udp-stack would migrate first because its raw-Ethernet IPC was the worst fit for the 256-byte envelope. What actually happened: the compositor → scanout-driver pixel path landed first (per [ADR-014 § Divergence 2026-04-20](014-compositor-scanout-driver-protocol.md)) because the rendering pipeline was the v1 critical path with a measured throughput requirement that the control IPC could not meet, and the libgui client → compositor surface channels followed shortly after (per [ADR-014 § Divergence 2026-05-08](014-compositor-scanout-driver-protocol.md)). virtio-net, udp-stack, and virtio-blk remain on control IPC as of this entry. The control-vs-data split this ADR established is still correct; the order of migration was driven by which workload's data plane was load-bearing for v1 shipping, which turned out to be pixels rather than packets.

  **Revisit when:** a networking workload's data plane saturates the 256-byte control envelope (measured: sustained queue-full audit events on a virtio-net or udp-stack endpoint, or a new workload — TCP bulk transfer, video over network, signed-carrier sensor stream — whose per-message size demonstrably exceeds the envelope). At that point the virtio-net ↔ udp-stack migration becomes the second channel consumer and the Implementation Notes' "first migration" performance baseline gets retroactively populated.

- **8. "See Also in CLAUDE.md" section is retired.** The four items in that section were forward-looking instructions at draft time. As of this Divergence entry:
  - § "Required Reading by Subsystem" - added; this ADR is cited under the "IPC bulk path (channels - Phase 3)" row.
  - § "Lock Ordering" - kept current via item 1 above.
  - § "Syscall Numbers" - the canonical list is `SyscallNumber` in [cambios-abi/src/lib.rs](../../cambios-abi/src/lib.rs) (re-exported through `src/syscalls/mod.rs`); CLAUDE.md no longer tabulates per-syscall numbers, so there is nothing here to keep in sync.
  - § "Post-Change Review Protocol" Step 8 - the protocol's Step 8 is now about numeric-bound tagging (Convention 8), not page-table sync. Documentation sync for ADR-touching changes is the §8 Documentation Sync category in [docs/review-protocol.md](../review-protocol.md), not a phase-specific table here.

  Future channel-substrate extensions (the Stream cap shape proposed in [ADR-030](030-stream-cap-variant.md), the deferred per-channel doorbell + `SYS_WAIT_CHANNEL` block primitive) will sync CLAUDE.md and the per-protocol design notes directly rather than via this section.

- **What did not change:**
  - The control-IPC / data-channel split is intact and load-bearing in every downstream ADR.
  - The 256-byte control envelope is preserved (re-confirmed under the 2026-05-01 PQ audit above).
  - The four boundary events (Create / Attach / Close / Revoke) remain the kernel's only contact with the data path for single-phase flows; two-phase teardown (item 3) adds two more boundary events for cluster revoke, not per-byte mediation.
  - The capability + identity composition is unchanged: channel caps live in the same `ProcessCapabilities` table, `sender_principal` stamping carries unmodified per [ADR-026](026-identity-transcription-at-the-kernel-ring.md).

- **Revisit when:** the next channel-substrate extension lands (the Stream cap-shape work per [ADR-030](030-stream-cap-variant.md), or a doorbell / `SYS_WAIT_CHANNEL` block primitive). At that point the drift catalog here should be re-audited against the new code shape; new drift gets a new appendix entry rather than amending this one.
