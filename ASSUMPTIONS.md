...
| `MAX_OBJECTS` (RamObjectStore) | 256 | [src/fs/ram.rs:22](src/fs/ram.rs#L22) | Phase 0 RAM-backed store, fixed-capacity array. | Disappears when persistent ObjectStore (Phase 4) lands — that backend will be dynamically sized. Until then, the first time we want to store >256 objects this is the wall. |
